...
def register():
    try:
        ...
        if request.method == 'POST':    
            ...
            if db.execute('SELECT id FROM activationlink WHERE email = ?', (email,)).fetchone() is not None:
...
def confirm():
    try:
        ...
        if request.method == 'POST': 
            ...
            db = get_db()
            attempt = db.execute(
                'SELECT * FROM forgolink WHERE id = ? and state = ?', (authid, utils.F_ACTIVE)
            ).fetchone()
            
            if attempt is not None:
                db.execute(
                    'INSERT INTO activationlink (state, id) VALUES (?, ?)', (utils.F_INACTIVE, attempt['id'])
                )
                salt = hex(random.getrandbits(128))[2:]
                hashP = generate_password_hash(password + salt)   
                db.execute(
                    'UPDATE user SET password = ?, salt = ?, to_id = ? WHERE = ?;', (hashP, salt, attempt['userid'])
                )
                db.commit()
                return redirect(url_for('auth.login'))
            else:
